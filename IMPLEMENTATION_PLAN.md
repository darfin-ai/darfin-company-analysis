...
5. DART 폴링 스케줄러 + 파이프라인 상태 머신; Spring 조회 엔드포인트; 프론트 목데이터 교체. **거의 완료** — Spring API/프론트 연동/스케줄러+큐까지 구현·라이브 검증됨.
   ...
   - **스케줄러 + 우선순위 큐 + 동시 LLM 호출 + 부분 렌더링** ✅ **완료** — 처음엔 "일일 스캔이 새 filing을 찾으면 자동으로 LLM 큐에 넣는다"로 만들었지만, 이러면 아무도 안 볼 회사까지 매일 Gemini 비용을 쓰게 된다는 지적(사용자)에 따라 **LLM 단계는 순수 on-demand로만 트리거하게 재설계**했다: `scripts/run_daily_scan.py`는 1~3단계(수집/파싱/재무제표/diff, LLM 없음)만 커버 대상 전체에 매일 실행하고 `llm_jobs`엔 아무것도 등록하지 않는다. `darfin-main`의 `GET /api/v1/companies/{corpCode}`가 `overview` 없는(=이 회사의 최신 filing이 아직 LLM 미처리) 회사를 조회할 때만 `llm_jobs`에 priority=0으로 등록/승격 — 이게 **유일한** 큐 등록 경로. `scripts/run_llm_worker.py`(cron이 1~2분마다 호출, 가장 급한 job 1개만 처리 — cron 주기가 자연스러운 Gemini rate limit)가 큐를 소비한다. 한 filing의 LLM 호출 4개(폴리싱/findings/risks/insights)는 `dart_pipeline/fast_path.py`에서 `ThreadPoolExecutor`로 동시 실행(서로 입출력이 안 겹쳐서 가능, 순차 대비 대략 2~3배 빠름).
     - **부분 렌더링**: LLM이 순수 on-demand가 되면서 "클릭했는데 아직 처리 전"이 예외가 아니라 일상이 됐다 — `financials`/`diffs`/`recentFilings`는 diff만 끝나면 이미 채워지므로(`diffs`는 diff 엔진이 만든 원문 before/after를 그대로 읽고, LLM 폴리싱이 되면 같은 컬럼이 다듬어진 텍스트로 덮어써질 뿐이라 폴리싱 전에도 원문이 이미 있음) 프론트가 페이지 전체를 막지 않고 `overview`에 의존하는 6개 패널 + findings 자리만 `shared/components/ui/skeleton.jsx`의 `Skeleton`으로 대체, 10~15초 폴링으로 완료되면 자동 교체.
