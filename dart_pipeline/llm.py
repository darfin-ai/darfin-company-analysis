...
mdnaHistory(사업 변화 흐름)는 이 모듈이 다루지 않는다 — LLM으로 "전략적
전환점"을 판정하려던 이전 버전(detect_strategy_shifts/detect_strategy_shift_increment,
삭제됨)이 재실행마다 비결정적으로 흔들리고 점진적 변화는 잘 놓쳐서, 판정
자체를 없애고 경영진 설명이 실제 있는 filing을 그대로 나열하는 결정론적
방식(overview.py: build_mdna_entry)으로 바꿨다.
"""
