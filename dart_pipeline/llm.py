...
strategyShifts(사업 변화 흐름)는 다른 5개 작업과 축이 다르다 — 한 filing 대
직전 baseline(QoQ) 비교가 아니라, 한 회사의 filing 역사 전체를 한 번에 보고
"사업의 중심축이 바뀐" 전환점을 찾는 질문이라 별도 함수(detect_strategy_shifts)
로 분리했다. 오케스트레이션은 strategy_shifts_ingest.py 담당.
"""
...
# ---------------------------------------------------------------------------
# 사업 변화 흐름(strategyShifts) 감지 — 여러 filing에 걸친 전략 전환점
# ---------------------------------------------------------------------------

_STRATEGY_SHIFT_SYSTEM_INSTRUCTION = (
    "당신은 한 기업의 여러 분기 공시를 시간순으로 검토해 사업 전략의 중대한 "
    "전환점을 찾아내는 보조 도구입니다. 입력은 filing_id(분기 표시)별로 정리된 "
    "요약 사실(재무 변동, 사업부문 매출 비중, 경영진 설명 발췌)입니다. 일부 "
    "filing에는 경영진 설명이 아예 없을 수 있습니다(분기·반기보고서는 이사의 "
    "경영진단 및 분석의견 기재 의무가 없어 원래 없는 경우가 많습니다).\n"
    "규칙:\n"
    "1. 입력에 없는 사실·수치를 절대 추가하지 마세요 — metrics의 수치는 "
    "반드시 입력에 등장한 값만 그대로 사용하세요.\n"
    "2. **경영진 설명이 실제로 있는 filing만 evidence_filing_id로 사용하세요.** "
    "재무 변동이나 매출 비중 변화가 크더라도, 그 filing에 경영진 설명이 없으면 "
    "그 변화만으로 전환을 보고하지 마세요 — 숫자 변동은 근거가 아니라 정황일 "
    "뿐입니다.\n"
    "3. 사소한 등락이 아니라 경영진 설명이 명시적으로 뒷받침하는, 사업의 "
    "중심축이 바뀐 명백한 전환만 보고하세요(예: 적자→흑자 전환, 주력 부문 "
    "교체, 신사업 급성장).\n"
    "4. 최대 3개까지만 시간순으로 보고하고, 조건에 맞는 게 없으면 빈 배열을 "
    "반환하세요 — 개수를 채우려고 억지로 만들지 마세요.\n"
    "5. rationale은 evidence_filing_id로 고른 filing에서 경영진이 실제로 밝힌 "
    "설명을 한국어 2~3문장으로 요약하세요(입력에 없는 이유를 추측하지 마세요)."
)


class _StrategyShiftMetric(BaseModel):
    label: str
    value_from: str | None
    value_to: str


class _StrategyShiftCandidate(BaseModel):
    evidence_filing_id: str
    prior_focus: str
    new_focus: str
    metrics: list[_StrategyShiftMetric]
    rationale: str


class _StrategyShiftBatch(BaseModel):
    shifts: list[_StrategyShiftCandidate]
...
class FilingDigest:
    ...
    quarter: str
    facts: str  # 이미 렌더링된 텍스트 블록(재무 변동/부문 비중/경영진 설명)
    has_management_rationale: bool = False  # 경영진 설명 실제 포함 여부(프롬프트 위반 시 기계적 필터링용)


@dataclass
class StrategyShiftCandidate:
    evidence_filing_id: str
    prior_focus: str
    new_focus: str
    metrics: list[dict]  # [{label, from, to}]
    rationale: str


def _digest_prompt(d: FilingDigest) -> str:
    return f"[filing_id={d.filing_id}, quarter={d.quarter}]\n{d.facts}"


def detect_strategy_shifts(client: genai.Client, digests: list[FilingDigest]) -> list[StrategyShiftCandidate]:
    """한 회사의 filing 요약을 전부 한 번에 보고 전략 전환점을 최대 3개까지 뽑는다.

    findings/risks와 달리 filing 단위가 아니라 회사 단위로 1회만 호출된다
    (오케스트레이션은 strategy_shifts_ingest.refresh_strategy_shifts_for_company).
    """
    if len(digests) < 2:  # 비교할 대상이 없으면 전환점도 없음
        return []

    # 경영진 설명이 실제로 있는 filing만 evidence로 인정 — 프롬프트로도
    # 지시하지만(규칙 2), 재무/비중 수치만 보고 임의로 골라 위반하는 경우가
    # 있어 결과를 받은 뒤 기계적으로도 한 번 더 걸러낸다.
    rationale_ids = {d.filing_id for d in digests if d.has_management_rationale}
    contents = "\n\n---\n\n".join(_digest_prompt(d) for d in digests)

    response = client.models.generate_content(
        model=MODEL_NAME,
        contents=contents,
        config=types.GenerateContentConfig(
            system_instruction=_STRATEGY_SHIFT_SYSTEM_INSTRUCTION,
            temperature=0.2,
            response_mime_type="application/json",
            response_schema=_StrategyShiftBatch,
            thinking_config=_NO_THINKING,
            max_output_tokens=_MAX_OUTPUT_TOKENS,
        ),
    )

    parsed: _StrategyShiftBatch | None = response.parsed
    if parsed is None:
        raise RuntimeError(
            f"Gemini 응답이 스키마로 파싱되지 않음 (finish_reason="
            f"{response.candidates[0].finish_reason if response.candidates else '?'}) — "
            f"digests={len(digests)}건"
        )

    results: list[StrategyShiftCandidate] = []
    for c in parsed.shifts:
        if c.evidence_filing_id not in rationale_ids:
            continue
        results.append(
            StrategyShiftCandidate(
                evidence_filing_id=c.evidence_filing_id,
                prior_focus=c.prior_focus,
                new_focus=c.new_focus,
                metrics=[{"label": m.label, "from": m.value_from, "to": m.value_to} for m in c.metrics],
