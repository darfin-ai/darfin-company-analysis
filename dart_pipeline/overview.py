...
mdnaHistory(사업 변화 흐름)도 이 모듈이 담당한다 — 원래는 LLM으로 "전략적
전환점"을 판정하려 했으나(strategy_shifts_ingest.py, 삭제됨), 재실행마다
비결정적으로 결과가 흔들리고(같은 이력에서 2건→1건으로 변하는 걸 실측) 강한
판정 프레이밍은 점진적 변화를 놓치는 문제가 겹쳐 폐기했다. 지금은 판정을
아예 안 한다 — 경영진 설명(MD&A)이 실제로 있는 filing을 시간순으로 그대로
나열해 사용자가 직접 읽고 판단하게 한다(LLM 호출 없음, 결정론적, 원문 그대로).
"""
...
# 분기·반기보고서는 "이사의 경영진단 및 분석의견" 기재 의무가 없어 정형 문구만
# 들어있다(실측: 삼성전자 기준 66자 고정 문구, 실제 서술이 있는 사업보고서는
# 1만자 이상) — 길이와 문구 매칭 둘 다로 걸러 실제 서술이 있는 filing만 인정.
_MDNA_MIN_LEN = 200
...
def build_mdna_entry(
    rcept_no: str, bsns_year: str, reprt_code: str, quarter: str, mdna_content: str | None
) -> dict | None:
    """이 filing에 실제 MD&A 서술이 있으면 타임라인 엔트리 1건을 만든다(없으면
    None — 분기·반기보고서 대부분이 여기 해당). 원문을 그대로 담고 LLM 요약은
    거치지 않는다."""
    content = (mdna_content or "").strip()
    if len(content) < _MDNA_MIN_LEN or _MDNA_BOILERPLATE_MARKER in content[:200]:
        return None
    return {
        "rceptNo": rcept_no,
        "quarter": quarter,
        "reportLabel": f"{bsns_year}년 {_REPRT_CODE_LABEL.get(reprt_code, reprt_code)}",
        "excerpt": content,
        "sourceRef": f"{rcept_no}#mdna",
    }
...
def _dividend_fiscal_year(bsns_year: str, reprt_code: str, column_index: int) -> tuple[str, bool]:
    """공시 표의 당기/전기/전전기 열을 회계연도로 매핑한다.

    분기·반기 보고서의 '당기' 열은 진행 중인 회계연도 누계(부분 기간)이고,
    '전기'·'전전기'는 완료된 전체 회계연도 값이다. 사업보고서의 '당기'만
    해당 연도 전체 배당을 의미한다.
    """
    year = int(bsns_year) - column_index
    is_partial = reprt_code != "11011" and column_index == 0
    return str(year), is_partial


def extract_dividend(
    dividend_chunk: dict | None,
    *,
    bsns_year: str,
    reprt_code: str,
) -> dict | None:
    ...
    상태로 이어받는다. history는 회계연도 기준으로 정렬하며, 분기·반기 보고서의
    당기(누계)는 isPartial=true로 표시해 연간 값과 혼동하지 않게 한다.
    """
            ...
            history.append(
                {
                    ...
                    # 구버전 프론트/캐시 호환 — 신규 UI는 fiscalYear를 쓴다.
                    "year": label,
                }
            )
